...
- **The committed default emits one schema per engine.** v2 uses `toolsSettings`, v3 uses `permissions.rules`, one or the other — measured on 2.18.1, where handing v2 a config carrying `permissions` made it drop the config entirely. A source reading of kiro-cli main / 2.21.1 revises this: the blessed shape is a dual-block profile (classic fields *plus* a derived `permissions` block), and #10733's silent drop of classic-only configs is intended (`is_kas_only_agent_config`), not a bug. `buildAgentConfig` can emit both blocks (`emitDualSchema`), but that path is gated off until it is re-measured on 2.21.1 with `kiro-perm-probe.yml`; do not flip the default without that measurement. See docs/security.md.
...
- **The CLI is spawned detached, and the run resolves on `exit` rather than `close`.** On kiro-cli 2.18.1, v3 leaked a KAS server as a grandchild that held the pipes; without the process group and the explicit pipe teardown the action finished its work and then hung. Fixed in 2.21.1 (kirodotdev/Kiro#10877), but the handling stays as a guard — do not remove it because a current CLI no longer needs it.
