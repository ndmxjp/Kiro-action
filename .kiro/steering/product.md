...
- The agent posts exactly one comment, plus — only when a workflow opts in with `use_inline_comments` on a pull request — up to 20 inline review comments. GitHub files those under a `COMMENTED` review, which carries no verdict; it still cannot approve or request changes.
