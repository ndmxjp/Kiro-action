...
- **The committed default emits one schema per engine.** v2 uses `toolsSettings`; v3 uses `permissions.rules`, one or the other — measured on 2.18.1, where handing v2 a config that carries `permissions` made it drop the config entirely and fall back to the user default. A source reading of kiro-cli main / 2.21.1 revises the target: the blessed shape is a dual-block profile (classic fields plus a derived `permissions` block, as `migration::upgrade_agent_config` produces), and #10733's silent drop of a classic-only config is intended (`is_kas_only_agent_config`), not a bug. `buildAgentConfig` can already emit both blocks behind `emitDualSchema`, but the default stays single-schema until the dual-block shape is re-measured on 2.21.1 with `kiro-perm-probe.yml`. Do not flip that default without the measurement.
...
- **The CLI child is spawned detached, and the run resolves on `exit`, not `close`.** On kiro-cli 2.18.1, v3 answered, exited, and left a KAS server as a grandchild holding the output pipes, so waiting for `close` never returned. The process group is signalled and the pipes are destroyed explicitly. Fixed upstream in 2.21.1 (kirodotdev/Kiro#10877); the handling stays as a guard against a regression or a pinned older CLI.
